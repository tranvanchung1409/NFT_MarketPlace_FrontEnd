...
describe('NFTMarketplace', function () {
  ...
  describe('Minting NFTs', function () {
    ...
    it('Should get all NFT owned', async function () {
      // addr1 mints an nft
      await nft.connect(addr1).mint(URI)
      await nft.connect(addr1).mint(URI)
      await nft.connect(addr1).mint(URI)
      ...
      console.log(await nft.getAllNFTOwned(addr1.address))
    })
  })
  ...
  describe('Making marketplace items', function () {
    ...
    it.only('Should return item on sale of the owner', async function () {
      await nft.connect(addr1).mint(URI)
      await nft.connect(addr1).mint(URI)
      await nft.connect(addr1).mint(URI)
      await nft.connect(addr1).mint(URI)
      ...
      console.log(await nft.getAllNFTOwned(addr1.address))
      console.log(await marketplace.getAllNFTOwned(addr1.address)) // []

      await marketplace.connect(addr1).makeItem(nft.address, 2, 1000)
      await marketplace.connect(addr1).makeItem(nft.address, 4, 1000)
      await marketplace.connect(addr1).makeItem(nft.address, 6, 1000)
      console.log(await marketplace.getAllNFTOwned(addr1.address)) // [2,4,6]
    })
